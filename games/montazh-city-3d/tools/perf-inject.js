...
(function () {
  ...
  function instrument(gl, attrs) {
    ...
    function account(target, w, h, layers, ifmt, levels) {
      ...
      /* то же для случая без мультисэмплинга: сцена пишет прямо в текстуру.
         ...
         Берём САМУЮ БОЛЬШУЮ подходящую, а не последнюю: буфер отражений
         (этап 05) — RGBA16F в половину стороны, восемь байт на тексель, и
         оговорку про три байта он проходит. Порядок создания тут ничего не
         значит, а размер значит: буфер сцены в кадре самый большой.

         Уже выбранная текстура при этом может УМЕНЬШИТЬСЯ: авто-масштаб
         перевыделяет её на каждом шаге. Поэтому «больше» проверяется только
         для чужих текстур, а свою мы узнаём по объекту и верим ей всегда. */
      if (target === gl.TEXTURE_2D && layers === 1 && levels === 1 &&
          fi.name.indexOf('DEPTH') < 0 && fi.bpp >= 3 && w >= 320 && h >= 240 &&
          (t === P._sceneTex || w * h >= (P.sceneW || 0) * (P.sceneH || 0))) {
        P.sceneW = w; P.sceneH = h; P._sceneTex = t;
      }
  ...
  P.PASS_NAMES = {
    ...
    /* Этап 05: размытие затенения — два прохода по одной оси, поэтому у него
       два порядковых номера. При QCFG.ssao.blurSep = false остаётся один. */
    'ssaoblur#0': 'размытие затенения: поперёк',
    'ssaoblur#1': 'размытие затенения: вдоль',
    'ssr#0': 'отражения в экранном пространстве',
    'main#0': 'статика района',
    'glow#0': 'тени под объектами',
    'main#1': 'персонажи и техника',
    'sky#0': 'небо',
    'glow#1': 'свечения и маркеры',
    /* Этап 05: порог свечения с уменьшением вдвое. Размытие делает
       generateMipmap, своей программы у него нет и в разбор он не попадает —
       его цена лежит внутри этой строки. */
    'bloom#0': 'свечение: порог и мип-цепочка',
