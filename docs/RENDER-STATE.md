...
Оснастка — пять файлов в `games/montazh-city-3d/tools/`, подробности и все
ключи в `tools/README.md`:
...
| `ext-probe.js` | Снимок возможностей WebGL2. Вставляется в консоль любого браузера — им снимается Safari, куда автоматика не дотягивается. |
| `toc.mjs` | Пересобирает оглавление в начале `index.html`. `--check` — проверка без записи. |

Как перезамерить целиком:
...
### Как снять Safari за пять минут
...
Тот же файл годится и для Chrome — сверить, что автоматика не наврала.

---

## 9. Chrome против Safari

**Сравнение не выполнено.** Причина не в лени и не в выборе: Safari существует
только на macOS, а замер идёт в линуксовом контейнере. Сборка WebKit для
Playwright (ближайший доступный суррогат) в этом окружении не установлена, а
`playwright install` запускать запрещено.

Оснастка к сравнению готова полностью — в `perf-probe.mjs` браузер выбирается
ключом:

```bash
node tools/perf-probe.mjs --browser=chromium --out=docs/perf/mac-chromium.json
node tools/perf-probe.mjs --browser=webkit   --out=docs/perf/mac-webkit.json
```

Оговорка, которую важно не потерять: `--browser=webkit` — это сборка WebKit из
Playwright, а не Safari. Движок тот же, но обвязка своя, и на Apple Silicon
путь к Metal у них может отличаться. Для настоящего Safari остаются два шага
вручную: расширения снимаются `ext-probe.js` через консоль (раздел 8), а время
кадра — вкладкой Timelines в веб-инспекторе на тех же трёх сценах.

Гипотезу, которую этот замер должен проверить, стоит записать заранее, чтобы
потом не подгонять: **Chrome на Apple Silicon идёт через ANGLE поверх Metal,
Safari обращается к Metal напрямую.** Лишний слой трансляции дорог там, где
много мелких вызовов и переключений состояния — а у нас на кадр приходится
27–43 отдельных вызова на тени, каждый со сменой программы и загрузкой двух
матриц (раздел 10, пункт 4). По этой части ожидание — выигрыш Safari. Зато
кадр упирается в заливку (67–74 % на одном проходе статики), а там решает
качество кода шейдера, и компилятор ANGLE→Metal вполне может оказаться не хуже
родного. Итог решается замером, а не рассуждением.
