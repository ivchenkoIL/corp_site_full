...
**Авто-масштаб на M4 не включается.** В headed на высоком профиле буфер
остаётся 2880×1800 при GPU 5.57 мс и кадре 16.7 мс. В headless контроллер
опускается до нижней планки 0.6 — это артефакт оснастки: замерная обвязка
держит единственный слот таймер-запроса, игра своего не открывает, а
накопленное за прогрев `gpuAvg` открывает ворота «GPU занят хотя бы
наполовину» для интервалов rAF, которые в headless гуляют от 7 до 26 мс. На
среднем профиле, где ворота закрыты, масштаб в том же headless не двигается.
