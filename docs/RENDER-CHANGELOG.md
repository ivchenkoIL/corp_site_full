...
Авто-масштаб: в headed-режиме на высоком профиле буфер остаётся 2880×1800,
GPU 5.57 мс при кадре 16.7 мс — **контроллер не включается, как и
задумано**. В headless он опускается до нижней планки 0.6, и это артефакт
оснастки: замерная обвязка держит единственный слот таймер-запроса, игра
своего не открывает, а накопленное за прогрев `gpuAvg` открывает ворота
«GPU занят хотя бы наполовину» для интервалов rAF, которые в headless гуляют
от 7 до 26 мс. Проверено: на среднем профиле, где GPU 3.6 мс и ворота
закрыты, масштаб в том же headless не двигается вовсе.
